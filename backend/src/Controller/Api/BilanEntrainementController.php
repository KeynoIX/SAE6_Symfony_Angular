...
{
    #[Route('/api/bilan-entrainement/{id}', name: 'api_bilan_entrainement', methods: ['GET'])]
    public function getBilanEntrainement(
        ...
        if (!$sportif) {
            return $this->json(['error' => 'Sportif non trouvé'], 404);
        }
        ...
        $nombreSeances = count($seanceRepository->findSeancesBySportifAndPeriod($id, $dateMinObj, $dateMaxObj));
        $repartitionTypes = $seanceRepository->getSeanceTypeDistribution($id, $dateMinObj, $dateMaxObj);
        $topExercices = $exerciceRepository->findTopExercicesBySportifAndPeriod($id, $dateMinObj, $dateMaxObj);
        $dureeTotal = $exerciceRepository->calculateTotalDuration($id, $dateMinObj, $dateMaxObj);
